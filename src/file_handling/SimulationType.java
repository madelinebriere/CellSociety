...
public abstract class SimulationType {
	...
	private void mapLists(Map<String, String> map, List<String> keys, List<String> values) {
		int element = 0; // Used to account for "cells" being skipped, as well
							// as having to access a List
		for (String tag : keys) {
			if (!tag.equals(CELLS)) {
	...
	protected List<String> createCellList(List<String> data) {
		if (getCellData().size() > 0) { // If given cell data is invalid, then
										// the default cell data will be used
			return data;
		} else {
