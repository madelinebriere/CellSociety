...
public class CellDataGenerator {
	...
	/**
	 * If the file gives the cell data in this format, then no changes need to
	 * be made and it is simply returned. Created primarily to make XMLParser
	 * more readable as it decides how to take the user's data and generate data
	 * that a SimulationType can use.
	 * 
	 * @return
	 */
	...
	/**
	 * Generates a list of Cells and their location given a Cell type and the
	 * number of those Cells desired. Assumes number of cells comes first, then
	 * cell type.
	 * 
	 * By making this public method call a private method, the same procedure
	 * can be done within this class without having to manipulate cellData, but
	 * the user does not need to pass in their List<String> again.
	 * 
	...
	private List<String> generateNumberData(List<String> dataList) {
		...
		for (String data : dataList) {
			...
			if (totalCells > dimension * dimension) { // Ensures the user didn't
														// ask for more cells
														// than possible, which
														// would create an
														// infinite loop
			...
			for (int i = 0; i < Integer.parseInt(splitData[0]); i++) { // A new
																		// Cell
																		// is
																		// added
																		// until
	...
	/**
	 * Generates a list of Cells and their location given a Cell type and the
	 * probability of that cell occurring. Assumes probability of cell comes
	 * first, then cell type.
	 * 
	 * @return List of cell locations and states
	 */
	public List<String> generateProbabilityData() {
		...
		if (probTotal > 1) { // Ensures that the given probabilities are valid
								// (Do not add up to more than 1)
	...
	/**
	 * Generates a list of Cells and their location given a Cell type and the
	 * percentage of the grid that cell should fill. Assumes percentage of cell
	 * comes first, then cell type.
	 * 
	 * To prevent duplication of code, the percentage data is converted to
	 * number data, then passed to generateNumberData.
	 * 
	 * @return List of cell locations and states
	 */
	...
	/**
	 * Generates random x and y values at which to place a Cell and creates a
	 * new String representing that data, placing it in the given List.
	 * 
	 * @param cellName
	 *            - name of Cell to be placed
	 * @param list
	 * @param filled
	 *            - 2d-array representing grid of cells, true if a cell is
	 *            there, false if not
	 */
	private void placeCellRandomly(String cellName, List<String> list, Boolean[][] filled) {
		...
		while (!placed) {
			...
			if (!filled[x][y]) { // Prevents two Cells from being placed in the
									// same location
	...
	/**
	 * Determines what Cell, if any, should be placed in the grid, according to
	 * a probability distribution.
	 * 
	 * @param list
	 *            - list to be filled with data
	 * @param names
	 *            - Array of cell names distributed by specific probabilities
	 */
	private void placeCellsWithProbability(List<String> list, String[] names) {
		...
		for (int x = 0; x < dimension; x++) {
			for (int y = 0; y < dimension; y++) {
				...
				if (!names[random].equals(null)) { // If it equals null, then
													// this no Cell will be
													// placed (empty)
	...
	/**
	 * Creates and returns a String[] holding the names of Cell types according
	 * to the probability that each occurs.
	 * 
	 * @param map
	 *            - Map holding the probabilities of each Cell
	 * @return
	 */
	private String[] createNamesArray(Map<String, Double> map) {
		...
		for (String cell : map.keySet()) { // Fills an array with Cell types
											// according to their probability
			end = (int) (Math.round(start + (100 * map.get(cell)))); // Sets
																		// probability
																		// to an
																		// integer
																		// between
