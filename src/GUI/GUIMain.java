...
public class GUIMain {
	...
	private void setupGrid() {
		_gridContainer = new Pane(); // controller will add its views to this
										// view
		...
		_gridController = new UIGridController(_gridContainer, new Frame(0, 0, GRID_WIDTH, GRID_WIDTH),
				_model.getCurrentColors(), _currentSimulationData);
	}
	...
	private void setupTopMenu() {
		...
		menu.setOnAction((event) -> {
			...
			_currentSimulationData = new SimulationData(new BoardData(name),
					SocietyMaker.getDefaultCellRatioValues(name));
			setFileToNull();
			resetAnimation();
		});
	...
	private void setupButtons() {
		...
		setButtonTheme(_fileButton, Color.rgb(220, 180, 30), Color.rgb(240, 200, 70), FontWeight.EXTRA_LIGHT, 11,
				BorderStrokeStyle.DASHED, 1);
	...
	private void setupShapeButtons(double topY) {
		...
		Polygon triangle = new Polygon(new double[] { x + width / 2, topY, x + width, bottomY, x, bottomY });
		x += width + 25;
		Rectangle square = new Rectangle(x, topY, 50, 50);
		double center = 25;
		double side = center / (Math.sqrt(3) / 2);
		x += 50 + 25 + side / 2;
		Polygon hexagon = new Polygon(new double[] { x, topY, x + side, topY, x + side + side / 2, topY + center,
				x + side, topY + center * 2, x, topY + center * 2, x - side / 2, topY + center });
	...
	/*
	 * private void setShapeDeselected(Shape s){ setShapeDesign(s, Color.rgb(80,
	 * 80, 80), 2);
	 * 
	 * }
	 */
	...
	private void setButtonToHighlightedState(Button button) {
		setButtonTheme(button, Color.rgb(70, 70, 70), Color.rgb(30, 30, 30), FontWeight.SEMI_BOLD, 14,
				BorderStrokeStyle.SOLID, 2);
	}

	private void setButtonToUnhighlightedState(Button button) {
		setButtonTheme(button, Color.DARKGRAY, Color.rgb(90, 90, 90), FontWeight.LIGHT, 14, BorderStrokeStyle.DASHED,
				1);
	}

	private void setButtonToSelected(Button button) {
		setButtonTheme(button, Color.rgb(0, 122, 255), Color.rgb(56, 56, 56), FontWeight.MEDIUM, 14,
				BorderStrokeStyle.SOLID, 2);
	}
	...
	private void setupSizeSlider(double sliderWidth, double startX, double startY) {
		...
		_sizeSlider.valueProperty().addListener((observable, oldValue, newValue) -> {
			...
			double minValue = Math.max(_currentSimulationData.getDimensions().getX(),
	...
	private void updateGenerationLabel() {
		_generationLabel.textProperty()
				.set("Generation " + ((Integer) _gridController.getCurrentGeneration()).toString());
	}

	/**
	 * updates grid using model (CellSociety object) to retrieve 2d-array of
	 * colors and current grid dimensions the the new dimensions will be used to
	 * dynamically change the grid size
	 */
	private void step() {
		updateGenerationLabel();
