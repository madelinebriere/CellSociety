...
/**
 * Extension of Patch specific to the Slime Society simulation. These patches
 * have memory in that they store a certain concentration (here, of cAMP). They
 * discharge this concentration at a certain evaporation rate, and have
 * deposited a certain amount of chemical in the patch if there is a Slime Cell
 * in the patch.
 * 
 * NOTE: This patch has a very active role -- it defines its concentration by
 * the Cell in it without the Cell having any role in the action. This is
 * because Patches are higher on the hierarchical chain and therefore take on
 * more responsibility.
 * 
 * @author maddiebriere
 *
...
public class SlimePatch extends Patch {
	...
	public SlimePatch(int col, int row, Color color) {
		this(row, col, color, EVAPO_RATE);
	}

	public SlimePatch(int row, int col, Color color, int evap) {
		super(row, col, color);
