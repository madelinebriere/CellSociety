 ...
 * are slightly higher on the hierarchical chain than Cells. They exist in every
 * valid spot and have functionality based on type. They either hold a Cell
 * (when a Cell has the same location) or hold no Cell and act as Color place
 * holders and sometimes sources of "memory." All patches are by default given a
 * concentration variable to represent this memory.
 * 
 * @author maddiebriere
 *
 */
...
public abstract class Patch {
	...
	public final static int MAX_CONC = 30;
	public final static int DEFAULT_CONC = 1;
	...
	private Cell myCell;
	private int concentration; // A variable for patches with "memory" -- can
								// have specific use
	...
	public Color getShadedColor() {
		double fraction = (concentration) / (1.5 * MAX_CONC); // anywhere
																		// from
																		// 0 to
																		// .25
		double shift = 1 - fraction; // Larger concentration --> lower
										// brightness
		if (shift > .9)
			shift = 1;
