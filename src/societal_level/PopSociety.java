...
/**
 * Extension of CellSociety specific to Segregation simulation, with multiple
 * populations relocating according to certain thresholds of satisfaction.
 * 
 * HUGE assumption: Only three types of populations are allowed. Our design
 * could facilitate as many populations as there are cells, but the only
 * difference between populations is color and this did not warrant a set of
 * different classes. Because different cells are distinguished by class name,
 * HouseCells of different colors must be manually accounted for (and assigned
 * colors).
 * 
 * @author maddiebriere
 *
